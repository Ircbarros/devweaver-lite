# DevWeaver-Lite - Claude Activation

> Two ways to use this skill with Claude:
>
> Option A - Claude Projects (recommended, richer):
>   1. Create a Claude Project at claude.ai.
>   2. Add Project Instructions: paste the content of this file.
>   3. Add Project Knowledge files: upload SKILL.md, librarian/CATALOG.md, adr/adr_lite_001_architecture.md.
>   4. Start a conversation. Tell Claude the path or description of the project you are working on.
>
> Option B - Single file (zero setup):
>   Paste this entire file as the system prompt or first message.
>   Tell Claude the path or description of the project you are working on.

---

## Claude Notes

- Context budget: 192 000 tokens (Claude 200 000 total - 8 000 reserved output).
- Tool approval: Claude asks for confirmation before tool calls in Claude Projects.
  This is the provider-level implementation of the CONFIRM gate and GIT GATE.
- If using Option A with Knowledge files: read SKILL.md and CATALOG.md from knowledge, not from file paths.
- If using Option B (inline): L3 references via model knowledge (no filesystem access in Claude.ai).
- Claude Desktop with MCPs: configure claude_desktop_config.json. See provider-specific/claude/claude_desktop_config.json.example.
- Tell Claude the project root path at the start of each session.
