# DevWeaver-Lite - OpenRouter Context

> Use this file as the system message when routing through OpenRouter.
> OpenRouter routes to multiple backend providers - context budget is model-dependent.
...
- Context budget: context_budget_remaining = 120000 (default - adjust per routed model).
- OpenRouter API: pass this file's content as the system message in /v1/chat/completions:
  messages=[{"role": "system", "content": <contents of this file>}, ...]
- Context budget is model-dependent. Check the routed model's context window at
  openrouter.ai/models and set context_budget = (model_context - 8000).
  Default 120 000 works for most 128K models. For 200K models (Claude), use 192000.
- Model selection: prefer models with >= 128K context for full skill operation.
  Models with < 32K context may truncate this file - verify the full content was accepted.
...
- L3 references: rely on model knowledge (no filesystem access via OpenRouter web interface).
